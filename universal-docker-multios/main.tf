...
variable "node_os" {
  type        = string
  description = <<-EOF
  Node OS type for compatibility (al2023, bottlerocket, ubuntu, auto)
  
  - al2023: Amazon Linux 2023 (default, recommended)
  - bottlerocket: AWS Bottlerocket (requires user namespaces enabled on nodes)
  - ubuntu: Ubuntu 20.04/22.04/24.04
  - auto: Automatically detect (uses node labels)
  EOF
  default     = "auto"
  validation {
    condition     = contains(["al2023", "bottlerocket", "ubuntu", "auto"], var.node_os)
    error_message = "node_os must be one of: al2023, bottlerocket, ubuntu, auto"
  }
}
...
locals {
  # Determine if we should mount host paths (not needed for Bottlerocket)
  mount_host_paths = var.node_os != "bottlerocket"
  
  # Node selector for auto mode (empty map means no restriction)
  node_selector = var.node_os == "auto" ? {} : {
    "coder.com/envbox-compatible" = "true"
  }
}
...
resource "coder_agent" "main" {
  ...
  metadata {
    display_name = "Node OS"
    key          = "node_os"
    script       = "cat /etc/os-release | grep '^ID=' | cut -d'=' -f2 | tr -d '\"' || echo 'unknown'"
...
resource "kubernetes_pod" "main" {
  ...
  spec {
    ...
    # Node selector based on OS choice (empty for auto mode)
    ...
    container {
      ...
      # Conditional: Only mount host paths on AL2023 and Ubuntu (not Bottlerocket)
      dynamic "volume_mount" {
        for_each = local.mount_host_paths ? [1] : []
        content {
          mount_path = "/usr/src"
          name       = "usr-src"
        }
    ...
    # Conditional: Only create host path volumes on AL2023 and Ubuntu (not Bottlerocket)
    dynamic "volume" {
      for_each = local.mount_host_paths ? [1] : []
      content {
        name = "usr-src"
